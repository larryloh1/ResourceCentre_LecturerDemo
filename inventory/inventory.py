...
class Inventory():
    ...
    def addLaptop(self, assetTag, description, os): #(add to inventory.py after addcamera class)
        # Check for correct values
        correct = True
    ...
    def getAvailableCamera(self): #whole of this method added to inventory.py
        output = ""
    ...
    def getAvailableLaptop(self): #whole of this method added to inventory.py
        output = ""
        output += "{:<10}{:<30}{:<10}{:<12}{:<10}\n".format("AssetTag", 
    ...
    def returnCamera(self, assetTag): #add to inventory.py
        success = False
        if len(assetTag) > 0:
            # Refactor (C): use findcamera()
            for i in self.cameraList:
    ...
    def returnLaptop(self, assetTag): #add to inventory.py
        success = False
        if len(assetTag) > 0:
            # Refactor (C): use findcamera()
            for i in self.laptopList:
